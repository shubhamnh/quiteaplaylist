    ...
    <main class="flex flex-col flex-grow items-center px-4 md:px-8 py-3 bg-gray-50">
        <div class="flex flex-col flex-grow max-w-2xl px-3 py-4 sm:py-8">
            ...
            <div v-for="(playlist, index) in playlists" class="relative h-auto min-h-[7rem] w-full p-3.5 sm:p-5 mb-4 sm:mb-6 rounded-lg shadow cursor-pointer bg-white border border-gray-200" @click="searchUrl(playlist.id)" :key="playlist.id">
                ...
                <button @click.stop="confirmDeletePlaylist(playlist.id, playlist.snippet.title, index)" class="absolute bottom-5 right-5 mx-1 rounded-full p-2 shadow md:shadow-lg border bg-white hover:bg-gray-100" title="Delete Playlist" >
            ...
            <Teleport to="body">
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                    <div v-if="deletePlaylistModal" @click.self="deletePlaylistModal = false" class="grid place-items-center h-full w-full fixed top-0 left-0 bg-blue-gray-300 bg-opacity-60">
                        <div class="flex flex-col p-4 mx-4 h-auto w-auto sm:max-w-md rounded-lg bg-white">
                            ...
                            <div class="mx-2">
                                ...
                                <p class="text-sm mb-6 text-gray-800">Are you sure you want to delete the playlist? This will also remove video details related to the playlist stored offline on your device.</p>
                            ...
                            <div class="flex flex-row justify-center mt-5 mb-3 space-x-9">
                                <button @click="deletePlaylistModal = false" class="px-4 py-1 rounded-full border shadow-sm hover:bg-gray-100">Cancel</button>
